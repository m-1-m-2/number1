<template>
  <div class='lottery28 absolute top bottom left right flex flex-col'>
    <!-- 百家乐 -->
    <!-- 280 -->
    <div class="h-50 w-full relative flex justify-center items-center">
      <div class="flex px-20 rounded-4 absolute top-4- z-index-50"
      ...
      <div class="w-inherit h-inherit relative">
        <div class="w-inherit h-70 absolute top left flex flex-col">
          <div class="h-16"></div>
          <div class="h-46 w-full flex">
            <div class="flex-1 flex flex-row-reverse relative">
              <div class="absolute top-12 fz-28 bolder cl-blue">PLAYER</div>
              <div class="w-34 h-46 box-border ba-1px mr-2 z-index-10 flex items-center justify-center">
                <POKER_bjl_trans :data="openAwardData_trans.msg.result.split('|')[0]"
                                 :index_key="0"
                                 v-if="POKER_bjl_trans_1"></POKER_bjl_trans>
              </div>
              <div class="w-34 h-46 box-border ba-1px mr-2 z-index-10 flex items-center justify-center">
                <POKER_bjl_trans :data="openAwardData_trans.msg.result.split('|')[0]"
                                 :index_key="1"
                                 v-if="POKER_bjl_trans_3"></POKER_bjl_trans>
              </div>
              <div class="w-46 h-46 mr-2 relative z-index-10">
                <div class="w-46 h-34 box-border ba-1px absolute bottom">
                  <POKER_bjl_trans :data="openAwardData_trans.msg.result.split('|')[0]"
                                   :index_key="2"
                                   v-if="POKER_bjl_trans_5"></POKER_bjl_trans>
                </div>
              </div>
            </div>
            <div class="flex-1 flex relative">
              <div class="absolute top-12 fz-28 bolder cl-red">BANKER</div>
              <div class="w-34 h-46 box-border ba-1px ml-2 z-index-10 flex items-center justify-center">
                <POKER_bjl_trans :data="openAwardData_trans.msg.result.split('|')[1]"
                                 :index_key="0"
                                 v-if="POKER_bjl_trans_2"></POKER_bjl_trans>
              </div>
              <div class="w-34 h-46 box-border ba-1px ml-2 z-index-10 flex items-center justify-center">
                <POKER_bjl_trans :data="openAwardData_trans.msg.result.split('|')[1]"
                                 :index_key="1"
                                 v-if="POKER_bjl_trans_4"></POKER_bjl_trans>
              </div>
              <div class="w-46 h-46 ml-2 relative z-index-10">
                <div class="w-46 h-34 box-border ba-1px absolute bottom">
                  <POKER_bjl_trans :data="openAwardData_trans.msg.result.split('|')[1]"
                                   :index_key="2"
                                   v-if="POKER_bjl_trans_6"></POKER_bjl_trans>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="flex-1 flex flex-col">
          <div class="flex-1 flex py-4 box-border"
            ...
            <div class="flex-1 px-6 box-border relative"
                 ...
                 :ref="'way' + k + '_' + key">
              <div class="w-full h-full rounded-60 box-border scroll-hide flex flex-col"
                ...
                <div class="flex-1 flex flex-col fz-12">
                  <div class="flex-1 flex items-center justify-center">{{val.st}}</div>
    ...
    <!-- 开奖弹框 -->
    <div class="w-300 h-90 box-border absolute rounded-4 flex flex-col px-10 z-index-20"
         v-if="!!openAwardData"
         style="border: 2px solid yellow; background: rgba(0,0,0,0.5); top: 50%; left: 50%; margin-left: -150px; margin-top:-50px;">
      <div class="h-30 w-full fz-14 cl-red line-30 flex justify-center">
        <div class="flex-1">{{openAwardData.msg.bjl.zhuangxianStr}}</div>
      ...
      <div class="h-50 w-full flex items-center justify-center">
        <div class="flex w-fit">
          <POKER_bjl :data="openAwardData.msg.result.split('|')[0]"
                     direction="left"></POKER_bjl>
          <div class="fz-20 line-42 cl-yellow px-10">vs</div>
          <POKER_bjl :data="openAwardData.msg.result.split('|')[1]"
                     direction="right"></POKER_bjl>
        </div>
      </div>
      <!-- 开奖声音 -->
      <audio autoplay
...
import POKER_bjl from "./exhibitResult/POKER_bjl";
import POKER_bjl_trans from "./exhibitResult/POKER_bjl_trans";
...
export default {
  name: 'lottery28',
  components: { lotteryBetHistory, POKER_bjl, chips, POKER_bjl_trans },
  ...
  data () {
    return {
      ...
      openAwardData: '',
      openAwardData_trans: '',
      POKER_bjl_trans_1: false,
      POKER_bjl_trans_2: false,
      POKER_bjl_trans_3: false,
      POKER_bjl_trans_4: false,
      POKER_bjl_trans_5: false,
      POKER_bjl_trans_6: false,
      countdownTime: '',
    ...
    openAwardData: {
      handler (newValue, oldValue) {
        ...
        if (!!newValue) {
          this.delayFn(() => {
            _this.openAwardData = '';
            _this.POKER_bjl_trans_1 = false;
            _this.POKER_bjl_trans_2 = false;
            _this.POKER_bjl_trans_3 = false;
            _this.POKER_bjl_trans_4 = false;
            _this.POKER_bjl_trans_5 = false;
            _this.POKER_bjl_trans_6 = false;
          }, 4000)
        }
      },
      deep: true,  // 深度监听
  ...
  methods: {
    ...
    initSocket () {
      ...
      this.mSocket.on(event, (rawResponse) => {
        ...
        if (msgID == 9 && result.msg.lotteryType == this.MOREGAMETYPE && this.MOREGAME) {
          // this.getters_getBetViewInfo.lastResult.open_result = result.msg.winWays[0].split('_')[1];
          this.openAwardData_trans = result;

          this.POKER_bjl_trans_1 = true;

          this.delayFn(() => {
            this.POKER_bjl_trans_2 = true;
          }, 500)
          this.delayFn(() => {
            this.POKER_bjl_trans_3 = true;
          }, 1000)
          this.delayFn(() => {
            this.POKER_bjl_trans_4 = true;
          }, 1500)

          this.delayFn(() => {
            let player_length = this.openAwardData_trans.msg.result.split('|')[0].split(":")[1].split(',').length;
            let banker_length = this.openAwardData_trans.msg.result.split('|')[1].split(":")[1].split(',').length;

            if (player_length == 3) { this.POKER_bjl_trans_5 = true }
            if (banker_length == 3) { this.POKER_bjl_trans_6 = true }
          }, 4000)
          // 延迟展示结果
          this.delayFn(() => { this.openAwardData = result; this.getters_getBetViewInfo.lastResult.open_result = result.msg.winWays[0].split('_')[1]; }, 5400)
          ...
          this.getters_getBetViewInfo.ways[0].options[4].betall = 0;
          ...
          this.getters_getBetViewInfo.ways[0].options[4].chouMa = [];
        ...
        if (msgID == 8 && result.msg.lotteryType == this.MOREGAMETYPE && this.MOREGAME) {
          ...
          } else if (way.includes('_闲对')) {
            this.getters_getBetViewInfo.ways[0].options[4].betall += Number(result.msg.ct.totalmoney);
            let top = Math.random() * 16 + 10;
            let left = Math.random() * 90;
            let offsetX = 110 * 3 - left;
            let offsetY = 118 - top;
            this.getters_getBetViewInfo.ways[0].options[4].chouMa.push({
              money: Number(result.msg.ct.totalmoney),
              top: `${top}px`,
              left: `${left}px`,
              offsetX: `${offsetX}px`,
    ...
    handler_open (result) {
      ...
      this.getters_getBetViewInfo.ways[0].options[4].betmine = 0;
    ...
    handler_user (result) {
      ...
      if (way.includes('_闲胜')) {
        ...
        this.getters_getBetViewInfo.ways[0].options[4].betall += Number(result.msg.ct.totalmoney);
        if (uid == this.UID) {
          this.getters_getBetViewInfo.ways[0].options[4].betmine += Number(result.msg.ct.totalmoney);
        }
        let top = Math.random() * 16 + 10;
        let left = Math.random() * 90;
        let offsetX = 110 * 3 - left;
    ...
    initChouMa () {
      ...
      if (this.getters_getBetViewInfo.ways[0].options[4].betall != 0) {
        this.getters_getBetViewInfo.ways[0].options[4].chouMa.push({
          money: Number(this.getters_getBetViewInfo.ways[0].options[4].betall),
          top: `${Math.random() * 16 + 10}px`,
          left: `${Math.random() * 90}px`,
        });
      }
  ...
  computed: {
    ...
    waysOption () {
      ...
      this.getters_getBetViewInfo.ways[0].options[4].optionName = this.getters_getBetViewInfo.ways[0].name;
      this.getters_getBetViewInfo.ways[0].options[5].optionName = this.getters_getBetViewInfo.ways[0].name;

      arr[0].push(this.getters_getBetViewInfo.ways[0].options[1]);
      arr[0].push(this.getters_getBetViewInfo.ways[0].options[2]);
      arr[0].push(this.getters_getBetViewInfo.ways[0].options[0]);
      arr[1].push(this.getters_getBetViewInfo.ways[0].options[4]);
      arr[1].push(this.getters_getBetViewInfo.ways[0].options[5]);
