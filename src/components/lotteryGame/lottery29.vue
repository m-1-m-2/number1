<template>
  <div class='lottery29 absolute top bottom left right flex flex-col'>
    <!-- 炸金花 -->
    <!-- 280 -->
    <div class="h-50 w-full relative flex justify-center items-center">
      <div class="relative fz-20 top-10-">
        <div class="flex px-20 rounded-4 relative z-index-50"
      ...
      <!-- history -->
      <div class="absolute right-2- top-4 h-30 w-64 box-border rounded-4 px-6"
        ...
        <div class="w-full h-full relative">
          ...
          <div class="h-16 w-full absolute bottom left relative cl-white rounded-4 fz-12 text-center line-16">
            <img class="h-16 w-20"
                 :src="result_array_img[getters_getBetViewInfo.lastResult.who_win]" />
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="flex-1 flex">
          ...
          <div class="flex-1 flex relative"
            ...
            <div class="flex-1 px-6 box-border"
              ...
              <div class="w-full h-56 rounded-60 box-border scroll-hide flex flex-col"
...
import POKER_zjh from "./exhibitResult/POKER_zjh";
...
export default {
  name: 'lottery29',
  components: { lotteryBetHistory, POKER_zjh, chips },
    ...
    initSocket () {
      ...
      this.mSocket.on(event, (rawResponse) => {
        ...
        if (msgID == 9 && result.msg.lotteryType == this.MOREGAMETYPE && this.MOREGAME) {
          // console.log(result);
          this.getters_getBetViewInfo.lastResult.who_win = result.msg.zjh.whoWin;
          ...
          this.delayFn(() => {
            if (result.msg.winWays[0].includes('玩家一')) {
              this.getters_getBetViewInfo.ways[0].options[1].chouMa.forEach(element => {
                element.left_bool = true;
              });
              this.getters_getBetViewInfo.ways[0].options[2].chouMa.forEach(element => {
                element.left_2_bool = true;
              });
            } else
              if (result.msg.winWays[0].includes('玩家二')) {
                ...
                if (result.msg.winWays[0].includes('玩家三')) {
                  this.getters_getBetViewInfo.ways[0].options[0].chouMa.forEach(element => {
  ...
  computed: {
    ...
    waysOption () {
      ...
      arr.push(this.getters_getBetViewInfo.ways[0].options[1]);
      arr.push(this.getters_getBetViewInfo.ways[0].options[2]);
