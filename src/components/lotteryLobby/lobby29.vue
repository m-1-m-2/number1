<template>
  <div class='lobby29 top bottom left right absolute flex flex-col'>
    <div class="w-full h-140 flex cl-white">
      ...
      <div class="flex-1 box-border pt-4">
        <div class="h-20 w-full relative flex justify-center items-center z-index-50">
          <div class="relative fz-20 top-10">
            <div class="flex px-20 rounded-4"
        ...
        <div class="h-76 flex">
          <div class="w-100 flex flex-col relative top-8 left-6-">
            <div class="zjh-bg absolute w-100 h-100 top-32- flex flex-col">
              ...
              <div class="flex-1 flex flex-col">
                ...
                <div class="h-50 relative">
                  <POKER_zjh v-if="showPoker"
                             :openAwardData='openAwardData_player1'
                             :index="0"></POKER_zjh>
                </div>
              </div>
            </div>
          </div>
          <div class="w-100 flex flex-col relative top-8 left-12-">
            <div class="zjh-bg absolute w-100 h-100 top-32- flex flex-col">
              <div class="h-32 flex items-end justify-center">
                <img class="h-24 w-30"
                     :src="player2" />
              ...
              <div class="flex-1 flex flex-col">
                ...
                <div class="h-50 relative">
                  <POKER_zjh v-if="showPoker"
                             :openAwardData='openAwardData_player2'
                             :index="1"></POKER_zjh>
                </div>
              </div>
            </div>
          </div>
          <div class="w-100 flex flex-col relative top-8 left-18-">
            <div class="zjh-bg absolute w-100 h-100 top-32- flex flex-col">
              <div class="h-32 flex items-end justify-center">
                <img class="h-24 w-30"
                     :src="player3" />
              ...
              <div class="flex-1 flex flex-col">
                ...
                <div class="h-50 relative">
                  <POKER_zjh v-if="showPoker"
                             :openAwardData='openAwardData_player3'
                             :index="2"></POKER_zjh>
                </div>
              </div>
            </div>
        ...
        <div class="h-40 w-full flex flex-col">
      ...
      <div class="w-32 flex flex-col">
        <div class="flex-1">
          <!-- history -->
          <div class="absolute right-4 top-4 h-36 w-60 box-border rounded-4"
               @click="isHistory = true;">
            <div class="w-full h-full relative">
              <img src="~@/assets/images/game_historys.png"
                   class="w-full h-full"
                   alt="">
              <div class="h-20 w-full flex items-center justify-center absolute bottom left relative cl-white rounded-4">
                <img class="h-16 w-20"
                     :src="result_array_img[getters_getHomeBetViewInfo.lastResult.who_win]" />
              </div>
            </div>
          </div>
        </div>
              ...
              <div class="px-10 box-border mb-4"
                ...
                <div class="h-30 flex items-center">
                  <div class="w-90 h-24 relative cl-white">
                    <img class="h-20 w-30"
                         :src="result_array_img[v.who_win]" />
                  </div>
                </div>
              </div>
            </van-list>
...
import POKER_zjh from "@/components/lotteryGame/exhibitResult/POKER_zjh";
import scrollDiv from "@/components/lotteryGame/widget/scrollDiv";
import { ways29 } from "./ways";
import { graphics29 } from "./graphics";
...
export default {
  name: 'lobby29',
  components: { lotteryBetHistory, chips, ways29, graphics29, POKER_zjh, scrollDiv },
  ...
  data () {
    return {
      ...
      loading: false,
      finished: false,
      result_array_img: [require(`@/assets/images/zjh_player_1.png`), require(`@/assets/images/zjh_player_2.png`), require(`@/assets/images/zjh_player_3.png`)],
      currentIssue: '',
      zstBool: false,
      zstList: [],
      // 动画开奖
      ...
      openAwardData_player1: '',
      openAwardData_player2: '',
      openAwardData_player3: '',
      showPoker: true,
      // 图片
      player1: require(`@/assets/images/zjh_player_1.png`),
      player2: require(`@/assets/images/zjh_player_2.png`),
      player3: require(`@/assets/images/zjh_player_3.png`),
      // 滚动集合
      liveMsgTop: [],
      // numberVar
      numberVar: 30,
  ...
  watch: {
    openAwardData: {
      handler (newValue, oldValue) {
        let _this = this;
        if (!!newValue) {
          _this.openAwardData_player1 = newValue;

          this.delayFn(() => {
            _this.openAwardData_player2 = newValue;
          }, 500)
          this.delayFn(() => {
            _this.openAwardData_player3 = newValue;
          }, 1000)

          this.delayFn(() => {
            _this.openAwardData = '';
            _this.openAwardData_player1 = '';
            _this.openAwardData_player2 = '';
            _this.openAwardData_player3 = '';
    ...
    initSocket () {
      ...
      this.mSocket.on(event, (rawResponse) => {
        ...
        if (msgID == 9 && result.msg.lotteryType == currentLotteryType) {
          ...
          // 动画相关
          this.getters_getHomeBetViewInfo.lastResult.who_win = result.msg.zjh.whoWin;
          this.openAwardData = result;

          this.delayFn(() => { this.changeTimeBoolean = true }, 6000)
        }
...
.lobby29 {
  ...
  .zjh-bg {
    background-image: url("~@/assets/images/zjh_bolder.png");
    background-size: 100% 100%;
    background-repeat: no-repeat;
  }
  .betStatusBar::before {
