<template>
  <div class='graphics27 absolute top left right bottom flex'
       ref="graphics27">
    <!-- 大小 -->
  ...
  methods: {
    ...
    transListFn () {
      ...
      this.list.forEach((el, key) => {
        // 无和值  需要处理豹子
        if (key == 0) {
          if (this.baozi.includes(el.spare_2[0])) {
            array[index] = [];
            array[index].push(el);
          } else {
            array[index] = [];
          ...
          if (this.baozi.includes(el.spare_2[0])) {
            array[index][array[index].length - 1]['die'] = el.spare_2[0].substr(el.spare_2[0].length - 1, 1);
          } else {
      ...
      // console.log(array)
      // 处理数组第一项为和值集合
      if (this.baozi.includes(array[0][0].spare_2[0])) { array.splice(0, 1) }
    ...
    render_zstList () {
      ...
      this.actions_getOpenHistory(params).then((res) => {
        if (res.code == 0) {
          ...
          let params = this.$refs.graphics27.scrollWidth;
