<template>
  <div class='ways29 fz-12 cl-white absolute top bottom left right'>
    <div class="flex flex-wrap px-2 box-border">
      <div v-for="(v, k) in list"
           class="p-all-2 box-border relative"
           :class="[`order-${k}`, k >= 3?'w-Equally-4 h-90': 'w-Equally-3 h-120']"
           :key="k + 'ways29'">
        <div class="bg-mask-white w-full h-full flex flex-col items-center justify-center rounded-8 relative"
          ...
          <div class="absolute"
               :class="[k >= 3?'bottom-10': 'bottom-16']">x{{v.value}}</div>
