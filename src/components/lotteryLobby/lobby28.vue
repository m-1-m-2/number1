<template>
  <div class='lobby28 top bottom left right absolute flex flex-col'>
    <div class="w-full h-140 flex cl-white">
      ...
      <div class="flex-1 box-border pt-8">
        <div class="h-50 w-full relative flex justify-center items-center">
          <div class="flex px-20 rounded-4 absolute top-2 z-index-50"
          ...
          <div class="absolute right-4 top-4 h-36 w-64 box-border rounded-4"
            ...
            <div class="w-full h-full relative">
              <img src="~@/assets/images/game_historys.png"
                   class="w-full h-full"
                   alt="">
              <div class="h-20 line-20 text-center fz-12 w-full absolute bottom left relative cl-white rounded-4"
                   v-if="getters_getHomeBetViewInfo.lotteryType == 28">
                {{getters_getHomeBetViewInfo.lastResult.open_result}}
              </div>
            </div>
    ...
    <div class="w-full h-140 absolute top left"
      ...
      <div class="w-300 h-90 box-border absolute rounded-4 flex flex-col px-10 z-index-20"
        ...
        <div class="h-30 w-full fz-14 cl-red line-30 flex justify-center">
          ...
          <div class="flex-1 text-right">{{ getters_getHomeBetViewInfo.lastResult.open_result }}</div>
        </div>
...
import POKER_bjl from "@/components/lotteryGame/exhibitResult/POKER_bjl";
import POKER_bjl_trans from "@/components/lotteryGame/exhibitResult/POKER_bjl_trans";
import scrollDiv from "@/components/lotteryGame/widget/scrollDiv";
import { ways28 } from "./ways";
import { graphics28 } from "./graphics";
...
export default {
  name: 'lobby28',
  components: { lotteryBetHistory, chips, ways28, graphics28, POKER_bjl, POKER_bjl_trans, scrollDiv },
  ...
  methods: {
    ...
    initSocket () {
      ...
      this.mSocket.on(event, (rawResponse) => {
        ...
        if (msgID == 8 && result.msg.lotteryType == currentLotteryType) {
          // console.log(result);
          // console.log(111);
        ...
        if (msgID == 9 && result.msg.lotteryType == currentLotteryType) {
          ...
          // 延迟展示结果
          this.delayFn(() => { this.openAwardData = result; this.getters_getHomeBetViewInfo.lastResult.open_result = result.msg.winWays[0].split('_')[1]; }, 5400)

          this.delayFn(() => { this.changeTimeBoolean = true }, 10000)
        }
    ...
    init_zstList () {
      let params = { page: 0, lottery_type: 28 }
